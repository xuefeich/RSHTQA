...
ARI_CLASSES_ = {"STP":0,"SUM":1,"DIFF":2,"TIMES":3,"DIVIDE":4,"AVERAGE":5}
IF_OPERATOR_CLASSES_ = {"SWAP_MIN_NUM": 0, "SWAP":1, "ADD": 2, "MINUS": 3, "MULTIPLY": 4, "DIVISION": 5, "PERCENTAGE_INC":6, "PERCENTAGE_DEC":7, "NONE": 8}
...
def _is_division(num_facts:list, answer):
    ...
    if num_facts[1] != 0:
        ...
        cands.append(round(100.0 * num_facts[0]/num_facts[1], 2))
    if num_facts[0] != 0:
        cands.append(round(num_facts[1]/num_facts[0], 2))
        cands.append(round(100.0 * num_facts[1]/num_facts[0], 2))
    return round(answer, 2) in cands
...
def get_operator_class(derivation:str, answer_type:str, facts:list, answer, mapping:dict, scale, OPERATOR_CLASSES):
    ...
    try:
        if answer_type == "span":
            ...
            if len(operators) == 1: # if only has one operator, use the one in the derivation
                if operators[0] == "/":
