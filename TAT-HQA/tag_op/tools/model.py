...
class TagopFineTuningModel():
    def __init__(self, args, network, state_dict=None, num_train_steps=1):
        ...
        # print('num update param orig', sum([p.numel() for p in self.network.parameters() if p.requires_grad]))
        # require_grad_cnt = 0
        # for n, p in self.network.named_parameters():
        #    if n.startswith("encoder."):
        #        p.requires_grad = False
        # print('num update param after', sum([p.numel() for p in self.network.parameters() if p.requires_grad]))
    ...
    def update(self, tasks):
        ...
        self.top2o_loss.update(output_dict["top2o_loss"], 1)
